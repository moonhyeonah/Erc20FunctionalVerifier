...
contract GolemNetworkToken {
    ...
    function GolemNetworkToken(address _golemFactory,
        ...
        if (_golemFactory == 0) revert("_golemFactory == 0");//throw;
        if (_migrationMaster == 0) revert("_migrationMaster == 0");//throw;
        if (_fundingStartBlock <= block.number) revert("_fundingStartBlock <= block.number");//throw;
        if (_fundingEndBlock   <= _fundingStartBlock) revert("_fundingEndBlock   <= _fundingStartBlock");//throw;
